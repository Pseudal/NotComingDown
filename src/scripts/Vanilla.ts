...
//! todo 883
//* spawnTracer(ent, 90, [3], 20, 2, 0, 2) entity/ mainly used for creep/ multiplicator : 1 down, 2 left, 3 right, 4 up/ Y position/ rest is RGB
...
if (ent.Type == 241){
    ...
    //let rotation = EntSprite.Rotation + 90;
      if(ent.Variant == 1)
        spawnTracer(ent, ent.SpriteRotation+90, [0.65, 1.35], 0, 2, 0, 0)
      else
    ...
    if(ent.ToNPC().State == 8 && (EntSprite.IsPlaying("Attack2Right") && EntSprite.GetFrame() < 25 && EntSprite.GetFrame() > 5))
        spawnTracer(ent, 90, [4], 50, 2, 0, 2)
    if(ent.ToNPC().State == 8 && (EntSprite.IsPlaying("Attack2Left") && EntSprite.GetFrame() < 25 && EntSprite.GetFrame() > 5))
        spawnTracer(ent, 90, [2], 50, 2, 0, 2)
    if(ent.ToNPC().State == 8 && (EntSprite.IsPlaying("Attack2Up") && EntSprite.GetFrame() < 25 && EntSprite.GetFrame() > 5))
        spawnTracer(ent, 90, [3], 50, 2, 0, 2)
    if(ent.ToNPC().State == 8 && (EntSprite.IsPlaying("Attack2Down") && EntSprite.GetFrame() < 25 && EntSprite.GetFrame() > 5))
